...
Минимальный host-side генератор на python (требует `scapy`):

```python
# tools/dev/send_frame.py
from scapy.all import Ether, sendp
frame = Ether(dst="02:80:19:11:22:33", type=0x88b5) / b"SPRINTER NICRX TEST"
sendp(frame, iface="en0", count=1, verbose=True)
```

В DSS запустить `NICRX.EXE` (он стартует polling и ждёт `ISR.PRX`), затем
выполнить скрипт на хосте. В MAME log ожидать:
...
```text
[X2] RX LEN=0040 SRC=.. TYPE=88B5
```

Если frame не доходит, сначала проверить:

- MAC-адрес назначения совпадает с тем, что напечатал MAME при старте;
- pcap-интерфейс совпадает с тем, в который шлёт scapy;
- macOS не блокирует исходящий L2-frame на Wi-Fi (см. примечание выше);
- `RCR` карты имеет `RCR.AB=1` (бит 2, broadcast accept), если шлётся
  broadcast.
...
- **BPF-права слетают после reboot** при ручном `chmod`. Постоянное решение —
  ChmodBPF.
- **RX через pcap получает все кадры интерфейса**, включая собственный
  трафик хоста. RX-фильтр в драйвере (по dst MAC и/или EtherType) обязателен
  на стейдже 5+, иначе DSS-утилита захлебнётся в широковещательном шуме.
