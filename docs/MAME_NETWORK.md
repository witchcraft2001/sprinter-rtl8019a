...
роутер). Конкретные адреса фиксировать в `config/NETSMPL.CFG` и в
...
## DLWIN3: эксперимент с окном TCP в три сегмента

На developer-дискете `DLWIN3.EXE` — отдельный вариант DLDIRECT с окном
4380 байт вместо 2920. Заголовок содержит `EXPERIMENT RXWIN=4380`.
MSS 1460, политика ACK и измерение времени прежние; записи на диск нет.
Обычные DLDIRECT, FTP/WGET и DLL не меняются.

После подключения свежего `distr/sprinter-rtl8019a.img` сравнить:
...
Для каждого запуска записать отдельный pcap на `feth1`, начиная до SYN,
с фильтром `tcp and host <IP Sprinter>`. Не менять URL, маршрут и скорость
эмуляции; не ставить MAME на паузу. Сохранить итоговый экран и счётчик
`packets dropped by kernel`. Проверить MSS 1460 и окно 4380 в SYN/обычных
ACK тестовой версии, тот же размер тела, `Integrity: OK` и `RESULT OK`.
Здесь Integrity означает проверку HTTP framing/длины, не хеша файла.

Сравнивать не только скорость, но и повторные передачи/ошибки. При таймауте
сохранить экран и захват; вернуться к обычному DLDIRECT. У трёх сегментов
меньше запаса RX-кольца, поэтому это не разрешение увеличивать окно у
файловых клиентов. Приёмка в MAME/на железе пока не выполнена.
Английская инструкция: `docs/DLSPEED.md`.
