...
## G. Пропускная способность receive-window (0.3.2)
...
```
python3 -u tools/dev/dlspeed_server.py --bind 192.168.7.1 --port 18081 --count 389579
```
...
Подставить реальный адрес peer-а из топологии pcap. Ожидаются точные 389579
байт, `Integrity: OK`, `RESULT OK` и скорость не ниже прежних примерно
54 KB/s. Параллельно сохранить pcap с TCP port 18081. В последовательности
ACK после закрытого окна 536 следующий активный `RECV` должен публиковать
окно 2680, потому что DLSPEED передаёт буфер 2144 байт. ACK не должен
