...
The `X` at line `3000:`, column 1 of slot 1 is the 32-byte register
block at I/O `0x300`.
...
other than the kit's default `0x300`.  Either rejumper the card
or wait for the `RTL_IOBASE` runtime override (followup #24);
the shipped driver currently hard-codes `0x300`.
...
|------|----------------------------------------|
| 0    | Success                                |
| 1    | Usage error (bad flag, bad hex, ...)   |
