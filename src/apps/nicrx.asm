...
PRX_TIMEOUT_MS	EQU 10000		; stage-5 manual receive wait
RDC_LOOPS	EQU 8000
...
; ------------------------------------------------------
; WAIT_PRX_LONG: poll ISR.PRX for PRX_TIMEOUT_MS.  ISA is
; open at entry/return, but is closed around every pacing delay
; so the 50 Hz system interrupt is never starved.
; CF=0 OK (PRX cleared), CF=1 timeout. Trashes A,HL,IX.
; ------------------------------------------------------
WAIT_PRX_LONG
	LD	IX,(RTL_BASE_PTR)
	LD	HL,PRX_TIMEOUT_MS
.LP
	LD	A,(IX+RTL_ISR_OFF)
	AND	ISR_PRX
	JR	NZ,.GOT
...
; ------- messages -------
MSG_BANNER	DB "RTL8019AS NICRX v",PACKAGE_VERSION,0
MSG_X0		DB "[X0] INIT ",0
MSG_OK		DB "OK",0
