...
TFTP_TIMEOUT
	...
	; banner and exit B=EX_SRV_ERR (the network was fine,
...
; Network-class failure that happened mid-transfer: the
; output handle (if any) must be closed, then dump NIC
; regs and exit B=EX_NET_ERR.
FAIL_NIC_FILE
	CALL	@RTL.SNAPSHOT_REGS
	CALL	@ISA.ISA_CLOSE
	CALL	CLOSE_OUT_FH
...
; ------- messages -------
MSG_BANNER	DB "RTL8019AS TFTP v",PACKAGE_VERSION,0
...
TFTP_BSS_END	EQU TFTP_FILE_BUF + TFTP_FILE_BUF_SIZE
	...
	ASSERT TFTP_BSS_END < RT_STACK_TOP - 0x0100
