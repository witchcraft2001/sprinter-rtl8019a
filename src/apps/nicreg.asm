...
; ------------------------------------------------------
STOP_SETUP
	; Patch the small tables that carry packet-RAM page expectations.  The
	; same test binary then exercises either runtime layout.
	LD	A,(RTL_TPSR_PAGE)
	LD	(STOP_CFG + 11),A
	...
	LD	(G2_TABLE + 1),A
	LD	A,(RTL_TPSR_PAGE)
	LD	(G2_TABLE + 1 + TBL_ENTRY_SIZE),A
	LD	HL,(RTL_BASE_PTR)
	LD	A,CR_PAGE0_STOP
	CALL	WR
	LD	DE,STOP_CFG
...
G6_DMA_START
	...
	LD	HL,(G6_ADDR_PTR)
	LD	(IX+RTL_RSAR0_OFF),L
	LD	(IX+RTL_RSAR1_OFF),H
	LD	A,0
G6_DMA_CMD EQU $-1
	LD	(IX+RTL_CR_OFF),A
	OR	A
...
G6_ADDR_PTR	EQU G6_RB + G6_LEN	; runtime TX-page DMA address
...
G6_RB		EQU G6_PAT + G6_LEN	; G6_LEN, what came back
	...
	ASSERT G6_ADDR_PTR + 2 <= RT_STACK_TOP - 0x200
